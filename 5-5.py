...
def save_face_encoding(user_id, face_encoding):
    ...
    try:
        # Define the directory for storing the face encodings
        FACE_ENCODINGS_DIR = "face_encodings"
        ...
        # Create the file path for saving the face encoding
        file_path = os.path.join(FACE_ENCODINGS_DIR, f"user_{user_id}.pkl")

        # Save the face encoding to the pickle file
...
def login_page():
    ...
    with col1:
        st.markdown("<h3 style='text-align: center; color: #000; margin-bottom: 2rem;'>🔐 Secure Login</h3>", 
                    unsafe_allow_html=True)
        ...
        if login_method == "Password":
            ...
            # Biometric login section (same for admin and employee)
            ...
            if img_file is not None:
                ...
                # Load face encodings (for both admins and employees)
                ...
                if matches:
                    ...
                    if user:
                        ...
                        # If admin, grant access to admin features
                        if user[2] == 'admin':
    ...
    with col2:
        ...
        st.image(
            r"C:\Users\sobia\PyCharmMiscProject\entudio\assets\Logo1-removebg-preview.png",
            width=400,
...
def employee_dashboard():
    ...
    menu = st.sidebar.selectbox(
        "Menu",
        ["Home", "Mark Attendance", "View Attendance", "Daily Report", "Salary Information", "Announcement", "Leave Management", "Profile",
         "Biometric Setup"]
    )
    ...
    elif menu == "Annoncemnt":
        st.subheader("📢 Company Announcements")
        st.title(f"Employee Dashboard - {st.session_state.username}")
        show_announcement_popup()  # Show popup (floating)
    ...
    # In the employee_dashboard() function, under the "Profile" menu section
    elif menu == "Profile":
...
def admin_dashboard():
    ...
    menu = st.sidebar.selectbox(
        "Menu",
        ["Home", "QR Attendance","User Management", "Attendance Management", "Leave Management", "Reports", "Project Reports", "Salary Management", "Employee Directory","Admin Directory","Admin Biometric Setup", "College Details",
         "Settings"],index=["Home","QR Attendance", "User Management", "Attendance Management","Leave Management","Reports","Project Reports", "Salary Management",  "Employee Directory","Admin Directory","Admin Biometric Setup","College Details", "Settings"].index(
        ...
        if st.session_state.get("user_role") != "admin":
            ...
            if img_file is not None:
                ...
                if not face_locations:
                    ...
                    # Add countdown timer
                    if 'qr_countdown' not in st.session_state:
                        st.session_state.qr_countdown = 300  # 5 minutes
    ...
    # college details end 
        ...
        tab1, tab2 = st.tabs(["Announcement", "Backup/Restore"])
            ...
            with cols[idx % 3]:
                with st.container():
                    ...
                    with st.expander("View Details"):
                        ...
                        # Add professional skills if available
...
# Update the get_user_info() function
def get_user_info(user_id):
    conn = sqlite3.connect('attendance_system.db')
    c = conn.cursor()
    ...
    if result:
        return {
            ...
            'role': result[6],
            'photo': result[7],
            'professional_skills': result[8]  # Added this line
        }
    return None
...
#------------------------
# Update the get_all_users() function
def get_all_users():
    try:
        conn = sqlite3.connect('attendance_system.db')
        query = """
...
# Add this function (or fix existing one)
...
def generate_qr_code():
    ...
    totp = pyotp.TOTP(pyotp.random_base32(), interval=300)  # 5min validity
...
def qr_attendance_page():
    ...
    # Verify token
    totp = pyotp.TOTP(st.secrets["qr_secret"])  # Store secret in secrets
